...
export async function POST(req: Request) {
  ...
  try {
    ...
    const result = await generateText({
      ...
      tools: {
        ...
        solvedCase: tool({
          ...
          parameters: z.object({
            epilogue: z
              .string()
              .describe(
                'Epilogo de la historia. Futuro de los personajes despues de que el investigador resuelva el caso.'
              ),
          }),
          //Funcion que se ejecuta cuando se llama a la tool
        ...
        speakWithNpc: tool({
          //Cuando llamar a la tool
          description:
            'se ejecuta cuando el jugador dice "Hablar con [personaje]"',
          parameters: z.object({
            name: z
              .string()
              .describe('Nombre del personaje. nunca usar nombres con tilde'),
            prompt: z
              .string()
              .describe(
                'Caracteristicas del persona y su rol en la historia. por ejemplo: "eres hector, una chico pescador que vive en la esquina. eras amigo de la victima llamada William"'
              ),
          }),
          //Funcion que se ejecuta cuando se llama a la tool
          execute: async ({ name, prompt }) => {
            ...
            const agentResponse: AgentResponse = await speakWithNpc(
              name,
              prompt,
              messages[name.toLowerCase()]
                ? (messages[name.toLowerCase()] as CoreMessage[])
                : ([] as CoreMessage[])
            );
    ...
    if (currentAgent === 'storyteller') {
      let textToReturn = '';
      let resultTextFormated = {
        paragraph: messageAgent,
        option1: 'Continuar investigando',
        option2: 'Informar a la policia',
        option3: '',
      };
      ...
      console.log(
        `Chat API's resultTextFormated: ${JSON.stringify(resultTextFormated)}`
      );
      return NextResponse.json(
        {
          agentResponse: textToReturn,
          formattedResponse: resultTextFormated,
          currentAgent,
          gameOver,
        },
        { status: 200 }
    ...
    //El mensaje de respuesta es vacío
      ...
      return NextResponse.json(
        { error: 'El texto de retorno está vacio' },
        { status: 400 }
      );
    }
  ...
  const result = await generateText({
    ...
    tools: {
      endConversation: tool({
        //Cuando llamar a la tool
        description:
          'Se ejecuta cuando termina la conversación, ya sea porque el investigador muere, resuelve el caso o se despide de ti ' +
          name,
        parameters: z.object({
          cause: z
            .enum(['muerte', 'crimen resuelto', 'despedida'])
            .describe(
              'Motivo del final de la conversación: muerte, crimen resuelto o despedida'
            ),
          resume: z
            .string()
