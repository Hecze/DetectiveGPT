...
export default function Endgame({ storyConclusion, storySummary }: { storyConclusion: string, storySummary: string }) {
    const restartGame = () => {
        location.reload()
    }
        ...
        <main className="text-white flex flex-col min-h-screen w-screen justify-center bg-[#18130F] overflow-hidden">
            <div className="m-3 md:w-1/2 md:m-auto">
                ...
                <div className="flex flex-col gap-2">
                    <Button className="bg-[#413a32] text-[#e9ddcf] py-6 font-semibold opacity-90 break-words whitespace-normal">Descargar reporte (PDF)</Button>
                    <Button onClick={restartGame} className="bg-[#413a32] text-[#e9ddcf] py-6 font-semibold opacity-90 break-words whitespace-normal">Resolver otro caso</Button>
                </div>
            </div>
